...
const char * const BITCOIN_CONF_FILENAME = "bitoz.conf";
const char * const BITCOIN_PID_FILENAME = "bitozd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bitoz" is a composite category enabling all Bitoz-related debug output
            if(ptrCategory->count(string("bitoz"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BITozCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BITozCore
    // Mac: ~/Library/Application Support/BITozCore
    // Unix: ~/.bitozcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BITozCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BITozCore";
#else
    // Unix
    return pathRet / ".bitozcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitoz.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bitoz.conf
